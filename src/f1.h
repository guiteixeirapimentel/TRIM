...
public:
    ...
    const State& X, const StateDot& Xdot, double mass, double deltaE, double deltaT) const
    {
        ...
        X.q*meanchord/(2.0*V), deltaE, dynPress, cS) + cThrust(V, deltaT))
        + (cG * sin(X.theta)) + (X.q*X.w);

    }

    double JacobianX1(double meanchord,
    const State& Xref, const StateDot& Xdotref, double massref, double deltaEref, double deltaTref) const
    {
        double delta = 1e-10;
        State X = Xref;
        
        X.u = Xref.u - delta;
        double res = operator()(meanchord, Xref, Xdotref, massref, deltaEref, deltaTref)- 
        operator()(meanchord, X, Xdotref, massref, deltaEref, deltaTref);
        res /= delta;  
    ...
    const State& Xref, const StateDot& Xdotref, double massref, double deltaEref, double deltaTref) const
    {
        ...
        double res = operator()(meanchord, Xref, Xdotref, massref, deltaEref, deltaTref)- 
        operator()(meanchord, Xref, Xdot, massref, deltaEref, deltaTref);
        res /= delta;  
    ...
    const State& Xref, const StateDot& Xdotref, double massref, double deltaEref, double deltaTref) const
    {
        double delta = 1e-10;        
        double res = operator()(meanchord, Xref, Xdotref, massref, deltaEref, deltaTref)- 
        operator()(meanchord, Xref, Xdotref, massref, deltaEref - delta, deltaTref);
        res /= delta;  
    ...
    const State& Xref, const StateDot& Xdotref, double massref, double deltaEref, double deltaTref) const
    {
        double delta = 1e-10;        
        double res = operator()(meanchord, Xref, Xdotref, massref, deltaEref, deltaTref)- 
        operator()(meanchord, Xref, Xdotref, massref, deltaEref, deltaTref - delta);
        res /= delta;  
